...
def mock_doubao():
    ...
    mock.text_to_speech = AsyncMock(return_value=b"mp3-binary-data")
    return mock
...
class TestVoiceSynthesize:
    async def test_synthesize_returns_audio(self, async_client, auth_token, mock_doubao):
        with patch("app.api.voice.get_doubao_client", return_value=mock_doubao):
            resp = await async_client.post(
        ...
        assert resp.content == b"mp3-binary-data"
